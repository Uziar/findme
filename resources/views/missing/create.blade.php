...
@section('content')
<div class="card">
    <h2 class="card-header">Fill up details for Missing person</h2>
    <form method="post" action="{{ route('storeMissing') }}" class="card-body">
        
        @csrf
        <h3>Missing Person's Information</h3>
        ...
        <div class="form-group">
            <label for="age">Age of missing person</label>
            <input id="age" type="text" name="age" class="form-control {{ $errors->has('age') ? "is-invalid" : "" }}" value="{{ old('age') }}" placeholder="Age of missing person">
            @if($errors->has('age'))
